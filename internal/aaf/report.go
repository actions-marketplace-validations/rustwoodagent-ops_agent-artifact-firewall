...
func renderText(result ScanResult) string {
	...
	if len(result.Findings) == 0 && len(result.SuppressedFindings) == 0 {
		return b.String()
	}
	...
	if len(result.SuppressedFindings) > 0 {
		...
		for _, f := range result.SuppressedFindings {
			...
			fmt.Fprintf(&b, "Why it matters: %s\n", f.Explanation)
			fmt.Fprintf(&b, "Recommendation: %s\n", f.Recommendation)
			fmt.Fprintf(&b, "Suppression reason: %s\n", f.Suppression.Reason)
		}
	}
...
func renderJSON(result ScanResult) (string, error) {
	payload := orderedJSONObject{
		...
		{Key: "suppressed_findings_count", Value: len(result.SuppressedFindings)},
		{Key: "parse_errors_count", Value: parseErrorCount(result.Artifacts)},
		{Key: "findings", Value: summarizeFindings(result.Findings)},
		{Key: "suppressed_findings", Value: summarizeSuppressedFindings(result.SuppressedFindings)},
	}
	...
	sarif := map[string]any{
		...
		"runs": []any{map[string]any{
			...
			"properties": map[string]any{
				...
				"suppressed_findings_count": len(result.SuppressedFindings),
			},
			"results": results,
		}},
	}
...
func summarizeSuppressedFindings(findings []SuppressedFinding) []map[string]any {
	out := make([]map[string]any, 0, len(findings))
	for _, f := range findings {
		item := summarizeFindings([]Finding{f.Finding})[0]
		item["suppression"] = map[string]any{
			"path":   f.Suppression.Path,
			"reason": f.Suppression.Reason,
		}
		if f.Suppression.RuleID != "" {
			item["suppression"].(map[string]any)["rule_id"] = f.Suppression.RuleID
		}
		out = append(out, item)
	}
	return out
...
func normalizeScanResult(result ScanResult) ScanResult {
	...
	normalized.SuppressedFindings = append([]SuppressedFinding(nil), result.SuppressedFindings...)
	...
	sort.SliceStable(normalized.SuppressedFindings, func(i, j int) bool {
		left := normalized.SuppressedFindings[i].Finding
		right := normalized.SuppressedFindings[j].Finding
		leftPath := cleanPath(firstNonEmpty(left.RelativePath, left.Path))
		rightPath := cleanPath(firstNonEmpty(right.RelativePath, right.Path))
		if leftPath != rightPath {
			return leftPath < rightPath
